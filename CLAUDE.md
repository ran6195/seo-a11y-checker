...
8. **WCAG Checks Suite** (`checks/`): Independent, incremental suite with one script per WCAG success criterion — each combines a deterministic automated check (axe-core and/or hand-written heuristics/DOM interaction) with an optional Claude-powered fallback for cases that need semantic or visual judgment. Includes a multi-page orchestrator and an aggregated HTML/PDF report generator. Currently covers 26 of the 50 WCAG 2.1 A/AA criteria plus 1 WCAG 2.2 AA criterion (2.5.8, Target Size Minimum — the only rule axe-core adds under the `wcag22aa` tag); see `checks/` directory listing for which, or `checks/CRITERI.md` for the full classification of all 50 WCAG 2.1 criteria.
...
- One script per WCAG success criterion, incrementally added (26/50 WCAG 2.1 A/AA so far, plus 1 WCAG 2.2 AA criterion — see `checks/` for the current list)
