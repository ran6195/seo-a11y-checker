...
8. **WCAG Checks Suite** (`checks/`): Independent, incremental suite with one script per WCAG success criterion — each combines a deterministic automated check (axe-core and/or hand-written heuristics/DOM interaction) with an optional Claude-powered fallback for cases that need semantic or visual judgment. Includes a multi-page orchestrator and an aggregated HTML/PDF report generator. Currently covers 26 of the 50 WCAG 2.1 A/AA criteria plus 2 WCAG 2.2 AA criteria (2.5.8 Target Size Minimum — the only rule axe-core adds under the `wcag22aa` tag — and 2.4.11 Focus Not Obscured, reusing the same Tab-walk infrastructure as 2.4.7/2.1.2/2.4.3); see `checks/` directory listing for which, or `checks/CRITERI.md` for the full classification of all 50 WCAG 2.1 criteria.
...
- One script per WCAG success criterion, incrementally added (26/50 WCAG 2.1 A/AA so far, plus 2 WCAG 2.2 AA criteria — see `checks/` for the current list)
