# Guida all'uso del Controllo Accessibilità per Windows 11

Questa guida spiega passo-passo come utilizzare lo strumento di controllo accessibilità dei siti web su Windows 11, anche se non hai esperienza tecnica.
...
2. Vai nella cartella del progetto (quella che contiene i file `a11y-cli.js`, `package.json`, ecc.)
...
## Parte 3: Come controllare l'accessibilità di un sito
...
**Esempio reale:**
```powershell
node a11y-cli.js https://www.comune.milano.it
```
...
```powershell
node a11y-cli.js https://esempio.it -f html
```

**Solo JSON (file dati per altri programmi):**
```powershell
node a11y-cli.js https://esempio.it -f json
```

**Dichiarazione di accessibilità italiana:**
```powershell
node a11y-cli.js https://esempio.it -f dichiarazione --org "Nome Ente" --email "contatti@ente.it"
```

**Report AGID Allegato 2:**
```powershell
...
```powershell
node a11y-cli.js https://esempio.it -f all
```

### Dare un nome al report
Per dare un nome specifico al file del report:
```powershell
node a11y-cli.js https://esempio.it -o mio-report
```

Il report si chiamerà `mio-report.html`, `mio-report.md`, ecc.
...
```powershell
node a11y-cli.js https://www.example.com -p 10 --headless -f html
```
Questo comando:
...
### Esempio 2: Dichiarazione di accessibilità per un ente pubblico
```powershell
node a11y-cli.js https://www.comune.esempio.it -f dichiarazione --org "Comune di Esempio" --email "accessibilita@comune.esempio.it" --phone "012-3456789"
```

### Esempio 3: Controllo completo con tutti i report
```powershell
node a11y-cli.js https://www.mysite.com --crawl -f all -o report-completo
```

---

## Parte 6: Dove trovare i report
...
oppure per le dichiarazioni di accessibilità:
```
C:\percorso\tuo\progetto\dichiarazioni\
```

### Come aprire i report

**Report HTML:**
- Vai nella cartella `reports` o `dichiarazioni`
...
## Parte 8: Opzioni della dichiarazione di accessibilità

Quando usi `-f dichiarazione`, puoi personalizzare il documento con queste opzioni:

| Opzione | Cosa significa | Esempio |
|---------|----------------|---------|
| `--org` | Nome dell'organizzazione | `--org "Comune di Roma"` |
| `--email` | Email di contatto | `--email "info@comune.it"` |
| `--phone` | Telefono (opzionale) | `--phone "06-12345678"` |
| `--pub-date` | Data pubblicazione sito | `--pub-date "15/03/2022"` |
| `--cms` | Sistema usato | `--cms "WordPress"` |

**Esempio completo:**
```powershell
node a11y-cli.js https://www.comune.esempio.it -f dichiarazione --org "Comune di Esempio" --email "accessibilita@comune.esempio.it" --phone "012-3456789" --pub-date "01/01/2023" --cms "Drupal"
```
...
node a11y-cli.js https://esempio.it -p 10 -f html --headless

# Dichiarazione accessibilità italiana
node a11y-cli.js https://esempio.it -f dichiarazione --org "Nome Ente" --email "contatti@ente.it"
...
node a11y-cli.js https://esempio.it --crawl -f all
```
